...
export function AppSidebar() {
  ...
  return (
    <aside
      className={cn(
        "h-screen sticky top-0 flex flex-col transition-all duration-300 ease-in-out border-r border-border/50 bg-sidebar",
        collapsed ? "w-20" : "w-72"
      )}
    >
      ...
      <Button
        variant="ghost"
        size="icon"
        className="absolute -right-3 top-16 z-10 h-6 w-6 rounded-full border border-border bg-background shadow-md hover:bg-secondary"
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center border border-primary/20">
              <span className="font-bold text-primary">
                {currentAgent.username?.charAt(0)?.toUpperCase() || "U"}
              </span>
            </div>
